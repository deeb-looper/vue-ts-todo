...
interface TodoStoreActions {
  ...
  updateTodos: (params: { id: string, checked: boolean, content: string }) => void
  getTodos: () => void
}
...
const addTodo = async (content: string): Promise<void> => {
  ...
  try {
    ...
    if (res.status === 201) {
      ...
      todoStore.$patch((state: { todos: Todo[], isAdding: boolean, isAddSuccess: boolean }) => {
...
const deleteTodo = async (id: string): Promise<void> => {
  try {
    ...
    if (res.status === 204) {
      todoStore.$patch((state: { todos: Todo[], isDeleting: boolean, isDeleteSuccess: boolean }) => {
  ...
  const todoStore = useTodoStore()
  todoStore.$patch((state: { todos: any[] }) => {
    const index = state.todos.findIndex(item => item.id === id);
    if (index === -1) return;
    state.todos.splice(index, 1);
  })
...
const updateTodos = async (params: { id: string, checked: boolean, content: string }): Promise<void> => {
  try {
    const todoStore = useTodoStore()
